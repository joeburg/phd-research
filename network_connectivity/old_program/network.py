...
class Network:
	...
	def __init__(self, inputfile, molType, q_relative):

		if molType not in ['EtOCS', 'OCSEt', 'EtOCSMe', '135Benz', '13Benz', '14Benz',\
							'SiO2', 'EtOCSMethyl', 'EtOCSVinyl', 'EtOCSPhenyl', 'TVS',\
							'TVSR1', 'TVSR2', 'TVSR3', 'TVSR4', 'TVSR6']:
	...
	def AnalyzeConnectivity(self):
		...
		if self.molType == 'SiO2':
			self.q = (1.0/4)*(0*NT0 + 1*NT1 + 2*NT2 + 3*NT3 + 4*NT4)
		else:
			self.q = (1.0/3)*(0*NT0 + 1*NT1 + 2*NT2 + 3*NT3)
		...
		bridged = ['EtOCS', 'OCSEt', '13Benz', '14Benz',]
		bridged_w_terminal_group = ['EtOCSMe', 'EtOCSMethyl', 'EtOCSVinyl', 'EtOCSPhenyl']
		hyperconnected = ['135Benz', 'TVS', 'TVSR1', 'TVSR2', 'TVSR3', 'TVSR4', 'TVSR6']

		if self.molType in bridged:
			self.p = (1.0/4)*(1.0 + 3*self.q)
			self.p_rel = (1.0/4)*(1.0 + 3*self.q_rel)
		elif self.molType in bridged_w_terminal_group:
			self.p = (1.0/4)*(1.0 + 2.5*self.q)
			self.p_rel = (1.0/4)*(1.0 + 2.5*self.q_rel)
		elif self.molType in hyperconnected:
			self.p = (1.0/5)*(2.0 + 3*self.q)
			self.p_rel = (1.0/5)*(2.0 + 3*self.q_rel)
		elif self.molType == 'SiO2':
			self.p = 1.0*self.q
			self.p_rel = 1.0*self.q_rel

		# comptue the mean Si network connectivity
		mSi_4 = ['EtOCS', 'OCSEt', 'EtOCSMe', 'EtOCSMethyl', 'EtOCSVinyl',\
					'EtOCSPhenyl', '13Benz', '14Benz', 'SiO2']
		mSi_5 = ['135Benz','TVS', 'TVSR1', 'TVSR2', 'TVSR3', 'TVSR4', 'TVSR6']

		if self.molType in mSi_4:
			self.mSi = 4*self.p
			self.mSi_rel = 4*self.p_rel
		elif self.molType in mSi_5:
			self.mSi = 5*self.p
			self.mSi_rel = 5*self.p_rel
	...
	def RelativeCondensationDegree(self):
		...
		if self.molType == 'SiO2':
			self.q_rel = (1.0/4)*(0*NT0 + 1*NT1 + 2*NT2 + 3*NT3 + 4*NT4)
		else:
			self.q_rel = (1.0/3)*(0*NT0 + 1*NT1 + 2*NT2 + 3*NT3)
	...
	def ComputeDensity(self):
		...
		Nprecursors = {'EtOCS' : 0.5*self.NSi,
						'OCSEt' : 0.5*self.NSi,
						'EtOCSMethyl' : 0.5*self.NSi,
						'EtOCSMe' : 0.5*self.NSi,
						'EtOCSVinyl' : 0.5*self.NSi,
						'EtOCSPhenyl' : 0.5*self.NSi,
						'135Benz' : (1.0/3.0)*self.NSi,
						'13Benz' : 0.5*self.NSi,
						'14Benz' : 0.5*self.NSi,
						'SiO2' : 1.0*self.NSi,
						'TVS' : (1.0/3.0)*self.NSi,
						'TVSR1' : (1.0/3.0)*self.NSi,
						'TVSR2' : (1.0/3.0)*self.NSi,
						'TVSR3' : (1.0/3.0)*self.NSi,
						'TVSR4' : (1.0/3.0)*self.NSi,
						'TVSR6' : (1.0/3.0)*self.NSi}
