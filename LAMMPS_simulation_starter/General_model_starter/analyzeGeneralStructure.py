...
# from utils import GetFileTypes, MakeResultsDirectory
#-------------------------------------------------------------------------------------------------#
#-------------------------------------------------------------------------------------------------#
#-------------------------------------------------------------------------------------------------#
...
def analyzeStructure(SimulationData):
	atomfile = SimulationData['atomfile']
	free_atom_ID = SimulationData['free_atom_ID']

	# get the data filenames
	# filenames = GetFileTypes(atomfile)

	data, atomstemplate, atomTypes,\
		bondTypes, angleTypes, dihedralTypes = LoadData(SimulationData)
	...
	# read in the pair potential parameters
	pairPotentialFile = SimulationData['pairPotentialFile']
	pair_potential = LoadPairPotential(pairPotentialFile)

	# get the data for the free atom 
	free_atom_charge = atomTypes[free_atom_ID][1]
	...
	Structure = {
		...
		'bondstemplate' 	: bondstemplate,
		'anglestemplate' 	: anglestemplate,
		'dihedralstemplate' : dihedralstemplate,
		'free_atom_data'	: (free_atom_ID, free_atom_charge)
	}
	...
	return Structure, Potentials

#-------------------------------------------------------------------------------------------------#

def LoadData(SimulationData):
	# read in the data
	atomTypesFile = SimulationData['atomTypesFile']
	bondTypesFile = SimulationData['bondTypesFile']
	angleTypesFile = SimulationData['angleTypesFile']
	dihedralTypesFile = SimulationData['dihedralTypesFile']
	atomfile = SimulationData['atomfile']
	...
	while True:
		...
		if fields:
			...
			atomTypes[int(atomID)] = (atom_type, float(charge))
		else:
			break
	f.close()
	...
	while True:
		...
		if fields:
			...
			bondTypes[bond_type] = (int(bond_type_ID), float(bond_coeff))
		else:
			break
	f.close()
	...
	while True:
		...
		if fields:
			...
			angleTypes[angle_type] = (int(angle_type_ID), float(angle_coeff))
		else:
			break
	f.close()
	...
	while True:
		...
		if fields:
			...
			dihedralTypes[dihedral_type] = (int(dihedral_type_ID), float(K1),\
											float(phi1), float(K2), float(phi2),\
											float(K3), float(phi3))
		else:
			break
	f.close()
...
def LoadPairPotential(pairPotentialFile):
	pair_potential = []

	f = open(pairPotentialFile)
	f.readline()
	f.readline()

	# atomID epsilon sigma Rc(vdw) Rc(coul)
	while True:
		fields = f.readline().strip().split()
		if fields:
			ID = int(fields[0])
			...
			Rc_vdw = float(fields[3])
			Rc_coul = float(fields[4])

			pair_potential.append((ID, epsilon, sigma,\
									Rc_vdw, Rc_coul))
		else:
			break
	f.close()
